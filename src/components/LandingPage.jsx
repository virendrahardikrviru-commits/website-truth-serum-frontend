...
if (user) {
  ...
  const scanId = result && result.scanId;

  if (!scanId) {
    setCopyFeedback('This report cannot be shared because its report ID is unavailable.');
    window.setTimeout(() => setCopyFeedback(null), 3000);
    return;
  }

  const reportUrl = `${window.location.origin}/report/${encodeURIComponent(scanId)}`;
  ...
  try {
    if (
      ...
      await navigator.share({
        ...
        text: 'View this Website Truth Serum risk report.',
        url: reportUrl,
      });
      return;
    }
    ...
    if (
      ...
      setCopyFeedback('Public report link copied to clipboard.');
      window.setTimeout(() => setCopyFeedback(null), 3000);
      return;
    }
  ...
  if (!scanId) {
    setCopyFeedback('This report cannot be copied because its report ID is unavailable.');
    window.setTimeout(() => setCopyFeedback(null), 3000);
    return;
  }
